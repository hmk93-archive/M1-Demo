...
{
	ReadMaterial();
	savePath = "../Assets/Materials/" + savePath + ".xml";
...
void ModelExporter::ExportMesh(string savePath)
{
	...
	savePath = "../Assets/Meshes/" + savePath + ".mesh";
	WriteMesh(savePath);
}
...
void ModelExporter::ExportClip(string savePath)
{
	for (UINT i = 0; i < scene->mNumAnimations; i++)
	{
		...
		string path = "../Assets/Clips/" + savePath + to_string(i) + ".clip";
		WriteClip(clip, path);
	}
}
...
void ModelExporter::ReadMaterial()
{
	for (UINT i = 0; i < scene->mNumMaterials; i++)
	{
		...
		material->ambient = Vector4(color.r, color.g, color.b, 1.0f);

		srcMaterial->Get(AI_MATKEY_COLOR_DIFFUSE, color);
		material->diffuse = Vector4(color.r, color.g, color.b, 1.0f);

		srcMaterial->Get(AI_MATKEY_COLOR_SPECULAR, color);
		material->specular = Vector4(color.r, color.g, color.b, 1.0f);

		srcMaterial->Get(AI_MATKEY_SHININESS, material->specular.w);

		srcMaterial->Get(AI_MATKEY_COLOR_EMISSIVE, color);
		material->emissive = Vector4(color.r, color.g, color.b, 1.0f);
