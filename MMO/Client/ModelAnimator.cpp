...
void ModelAnimator::ReadClip(string file, bool loopValue)
{
	file = "../Assets/Clips/" + file + ".clip";
...
void ModelAnimator::CreateTexture()
{
	...
	{//CreatTexture
		...
		if (FAILED(Device::Get().GetDevice()->CreateTexture2D(&desc, subResource, &texture)))
		{
			__debugbreak();
		}
	...
	{//Create SRV
		...
		if (FAILED(Device::Get().GetDevice()->CreateShaderResourceView(texture, &desc, &srv)))
		{
			__debugbreak();
		}
	}
